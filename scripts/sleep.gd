...
func _physics_process(delta):
	...
	if int(timeText) == 0:
		_on_Timer_timeout()
		get_tree().change_scene("res://insideHouseAwake.tscn")
...
func _on_Timer_timeout():
	if not energyAdded:  # Check if energy has already been added
		var randomValue = 0
		if randi() % 2 == 0:
			randomValue = 50
		else:
			randomValue = 80
		Ui.restValue += randomValue
		print("Energy increased by: ", randomValue)
		timeText = 5
