...
#status and text to display
onready var ok = $Ok
onready var status = $Status

#containers for the box to display
...
#make a randomized placement for each boxes
...
#assign if the box is placed to the right container
var okText = ""
var okText2 = ""
var okText3 = ""
var okText4 = ""
...
func _physics_process(delta):
	...
	#assign color for clear placement
	...
	#if timer stops, make all color disappear
	if int(timer) == 0:
...
#placement for each panel to its respective container
func _on_Panel_gui_input(event):
	if event is InputEventScreenDrag:
		panel.rect_position += event.relative
	
	#if the panel is near to the container, snap it by making the position of the panel equal to the conatainer
	...
	#provide the text 'match' if the panel placed to its respectful container
...
func _on_TextureButton_pressed():
	if verified:
		...
		Ui.passedGuitar  = -1
	get_tree().change_scene("res://insideAudition1.tscn")
	pass # Replace with function body.
